...
module.exports.onCreateNode = function ({
  ...
  if (!_.isUndefined(pluginOptions.converterFactory)) {
    asciidoctor.ConverterFactory.register(new pluginOptions.converterFactory(asciidoctor), ['html5']);
  }
  ...
  return loadNodeContent(node).then(content => {
    ...
    const contentDigest = crypto.createHash(`md5`).update(doc.reader.source_lines.join('\n')).digest(`hex`);
    const html = doc.convert();

    const tags = _.compose([_.defaultTo([]), _.map(_.trim), _.filter(_.negate(_.isEmpty)), _.split(';')])(doc.getAttribute('tags'));
    ...
    const asciidocNode = {
      ...
      children: _.compose([_.filter(_.negate(_.isNull)), _.filter(_.negate(_.startsWith('/'))), _.filter(isRelative)])(_.concat(images, links)),
      html,
      parent: node.id,
      internal: {
        content,
